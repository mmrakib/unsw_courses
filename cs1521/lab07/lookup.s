...
main:
    ...
    # PUT YOUR CODE HERE
    move $s0, $t0           # value of x
    move $s1, $t1           # value of y

    la $t0, array            # $t0 = address of array grid
    li $t7, 24
    mul $t1, $t7, 4         # $t1 = size of each row in bytes
    mul $t2, $s0, $t1       # $t2 = byte offset to start of ith row
    add $t3, $t0, $t2       # $t3 = address to start of ith row
    mul $t4, $s1, 4         # $t4 = byte offset to jth col
    add $t5, $t3, $t4       # $t5 = address of jth col at ith row

    lw $a0, ($t5)         # printf("%d", 42)
    li $v0, 1
    syscall
