...
void access_page(int virtual_page, int access_time, int n_physical_pages, struct ipt_entry *ipt) {
    ...
    static int physical_page = 0;

    if (ipt[physical_page]->virtual_page == -1 && physical_page < n_physical_pages) {
        ipt[physical_page]->virtual_page = virtual_page;
        ipt[physical_page]->last_access_time = access_time;

        printf("Time %d: virtual page %d loaded to physical page %d\n", access_time, virtual_page, physical_page);
    } else if (ipt[physical_page]->virtual_page == -1 && physical_page >= n_physical_pages) {
        ipt[physical_page]->virtual_page = 
    } else if (ipt[physical_page]->virtual_page == virtual_page) {
