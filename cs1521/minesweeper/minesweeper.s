...
reveal_grid__body:
        ...
        reveal_grid__loop1_step:
        bge     $s0, N_ROWS, reveal_grid__loop1_end
        li      $s1, 0

        reveal_grid__loop2_step:
        bge     $s1, N_COLS, reveal_grid__loop2_end

        mul     $t0, $s0, N_COLS        #
        add     $t0, $t0, $s1           #
        lb      $t1, grid($t0)          # $t1 = grid[row][col]

        ori     $t1, $t1, IS_RVLD_MASK  #
        sb      $t1, grid($t0)          # grid[row][col] |= IS_RVLD_MASK;

        addi $s1, $s1, 1
        j       reveal_grid__loop2_step

        reveal_grid__loop2_end:

        addi $s0, $s0, 1
        j       reveal_grid__loop1_step

        reveal_grid__loop1_end:
...
place_bombs__body:
        ...
        place_bombs__loop1_step:
        lw      $t0, total_bombs 
        bge     $s0, $t0, place_bombs__loop1_end

        jal     place_single_bomb

        addi    $s0, $s0, 1
        j       place_bombs__loop1_step

        place_bombs__loop1_end:
...
mark_cell__body:
        ...
        mul     $t0, $s0, N_COLS        # if ((grid[row][col] & IS_RVLD_MASK))
        add     $t0, $t0, $s1           #
        lb      $t0, grid($t0)          #
        andi    $t0, $t0, IS_RVLD_MASK  #
        beqz    $t0, mark_cell__if1_false

        lw      $t0, debug_mode           #
        beqz    $t0, mark_cell__if2_false # if (debug mode)

        j       mark_cell__epilogue

        mark_cell__if2_false: 
        ...
        mark_cell__if1_false:


        mul     $t0, $s0, N_COLS        # if ((grid[row][col] & IS_MRKD_MASK))
        add     $t0, $t0, $s1           #
        lb      $t0, grid($t0)          #
        andi    $t0, $t0, IS_MRKD_MASK  #
        beqz    $t0, mark_cell__if3_else
        ...
        not     $t2, $t2                #
        and     $t1, $t1, $t2           #
        sb      $t1, grid($t0)          # grid[row][col] &= ~IS_MRKD_MASK;

        lw      $t0, bomb_count         #
        addi    $t0, $t0, 1             #
        sw      $t0, bomb_count         # bomb_count++;
        j       mark_cell__if3_end
...
reveal_cell__body:
        ...
        lb      $t0, grid($t0)          #
        andi    $t0, $t0, IS_MRKD_MASK  #
        beqz    $t0, reveal_cell__if1_false

        lw      $t0, debug_mode             #
        beqz    $t0, reveal_cell__if2_false # if (debug mode)

        j       reveal_cell__epilogue

        reveal_cell__if2_false: 
        ...
        lb      $t0, grid($t0)          #
        andi    $t0, $t0, IS_RVLD_MASK  #
        beqz    $t0, reveal_cell__if3_false

        lw      $t0, debug_mode             #
        beqz    $t0, reveal_cell__if4_false # if (debug mode)

        j       reveal_cell__epilogue

        reveal_cell__if4_false: 
        ...
        reveal_cell__if3_false: 


        mul     $t0, $s0, N_COLS        # if ((grid[row][col] & IS_BOMB_MASK))
        add     $t0, $t0, $s1           #
        lb      $t0, grid($t0)          #
        andi    $t0, $t0, IS_BOMB_MASK  #
        beqz    $t0, reveal_cell__if5_false

        li      $t0, LOSE               #
        sw      $t0, game_state         # game_state = PLAYING;

        reveal_cell__if5_false:        


        mul     $t0, $s0, N_COLS        # if (((grid[row][col] & VALUE_MASK) == 0))
        add     $t0, $t0, $s1           #
        lb      $t0, grid($t0)          #
        andi    $t0, $t0, VALUE_MASK    #
        bnez    $t0, reveal_cell__if6_else

        move    $a0, $s0
        move    $a1, $s1
        jal     clear_surroundings
        j       reveal_cell__if6_end

        reveal_cell__if6_else:  
        ...
        lw      $t1, game_state
        li      $t2, LOSE
        beq     $t1, $t2, reveal_cell__if7_false

        lw      $t1, cells_left         #
        addi    $t1, $t1, -1            #
        sw      $t1, cells_left         # cells_left--;

        reveal_cell__if7_false:

        reveal_cell__if6_end:


        lw      $t1, cells_left
        bnez    $t1, reveal_cell__if8_false

        li      $t1, WIN
        sw      $t1, game_state

        reveal_cell__if8_false:
...
clear_surroundings__body:
        ...
        bgez    $s0, clear_surroundings__if1_false
        li      $t0, N_ROWS
        blt     $s0, $t0, clear_surroundings__if1_false
        bgez    $s1, clear_surroundings__if1_false
        li      $t0, N_COLS
        blt     $s0, $t0, clear_surroundings__if1_false

        j       clear_surroundings__epilogue

        clear_surroundings__if1_false:

        mul     $t0, $s0, N_COLS        # if ((grid[row][col] & IS_RVLD_MASK))
        add     $t0, $t0, $s1           #
        lb      $t0, grid($t0)          #
        andi    $t0, $t0, IS_RVLD_MASK  #
        beqz    $t0, clear_surroundings__if2_false

        j       clear_surroundings__epilogue

        clear_surroundings__if2_false:

        mul     $t0, $s0, N_COLS        #
        add     $t0, $t0, $s1           #
        lb      $t1, grid($t0)          # $t1 = grid[row][col]
                                        # This line keeps throwing exceptions!?
                                        # It says "Bad address in data/stack" but after
                                        # many hours I still haven't figured out why 
                                        # this array access doesn't work here but works 
                                        # everywhere else
        ...
        mul     $t0, $s0, N_COLS        # if ((grid[row][col] & VALUE_MASK))
        add     $t0, $t0, $s1           #
        lb      $t0, grid($t0)          #
        andi    $t0, $t0, VALUE_MASK    #
        beqz    $t0, clear_surroundings__if3_false

        j       clear_surroundings__epilogue

        clear_surroundings__if3_false:

        move $a0, $s0
        move $a1, $s1

        addi $a0, $a0, -1       # row - 1, col
        jal clear_surroundings

        addi $a1, $a1, -1       # row - 1, col - 1
        jal clear_surroundings

        addi $a1, $a1, 2        # row - 1, col + 1
        jal clear_surroundings

        addi $a0, $a0, 1        #
        addi $a1, $a1, -2       # row, col - 1
        jal clear_surroundings
...
update_highscore__body:
        ...
        la      $t0, high_score
        lw      $t1, 0($t0)
        bge     $t1, $s0, update_highscore__if1_false

        sw      $s0, 0($t0)

        li      $s1, 0
        update_highscore__loop1_step:
        lb      $t0, user_name($s1)
        beqz    $t0, update_highscore__loop1_end

        la      $t1, high_score         # Start of high_score struct
        la      $t1, 4($t1)             # Start of high_score.name
        add     $t1, $t1, $s1           # high_score.name[i]
        lb      $t0, user_name($s1)     # user_name[i]
        sb      $t0, ($t1)              # high_score.name[i] = user_name[i]

        addi    $s1, $s1, 1
        j update_highscore__loop1_end

        update_highscore__loop1_end:

        la      $t1, high_score         # Start of high_score struct
        la      $t1, 4($t1)             # Start of high_score.name
        add     $t1, $t1, $s1           # high_score.name[i]
        sb      $0, ($t1)               # high_score.name[i] = '\0'

        li      $v0, TRUE
        j       update_highscore__epilogue

        update_highscore__if1_false:
...
print_scores__body:
        ...
        li $s0, 0
        print_scores__loop1_step:
        bge $s0, MAX_SCORES, print_scores__loop1_end

        mul     $t0, $s0, USER_SCORE_SIZE                                                      #
        la      $t0, scores($t0)
        lw      $t1, 0($t0)                     # int score
        la      $t2, 4($t0)                     # char name[MAX_NAME_LEN]

        bne     $t1, -1, print_scores__if1_false

        j       print_scores__loop1_end

        print_scores__if1_false:
        ...
        addi $s0, $s0, 1
        j print_scores__loop1_step

        print_scores__loop1_end:
