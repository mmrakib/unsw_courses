...
reveal_grid__body_loop1_step:
        bge $s0, N_ROWS, reveal_grid__body_loop1_end
        li $s1, 0       # Reset col back to 0

reveal_grid__body_loop2_step:
        bge $s1, N_COLS, reveal_grid__body_loop2_end
...
place_bombs__body:
        ...
        place_bombs__body_loop_step:
        la $s1, total_bombs                      # $s1 = address of total_bombs
        lw $s2, ($s1)                            # $s2 = value at address of total_bombs
        bge $s0, $s2, place_bombs__body_loop_end # (i < total_bombs)

        jal place_single_bomb                    # place_single_bomb(bad_row, bad_col)

        addi $s0, $s0, 1                         # i++
        j place_bombs__body_loop_step            # repeat loop

        place_bombs__body_loop_end:
...
mark_cell__body:
        ...
        lb $t6, ($t5)                             # grid[row][col]
        andi $t7, $t6, IS_RVLD_MASK
        beq $t7, FALSE, mark_cell__body_if1_false # if (grid[row][col] & IS_RVLD_MASK)

        mark_cell__body_if1_true:
        la $t8, debug_mode                        
        lw $t9, ($t8)
        beq $t9, FALSE, mark_cell__body_if2_false # if (debug_mode)

        mark_cell__body_if2_true:
        j mark_cell__epilogue                    

        mark_cell__body_if2_false:
        la $a0, mark_error
        li $v0, 4
        syscall
        j mark_cell__epilogue

        mark_cell__body_if1_false:

        lb $t6, ($t5)
        andi $t7, $t6, IS_MRKD_MASK
        beq $t7, FALSE, mark_cell__body_if3_false

        mark_cell__body_if3_true:
        lb $t6, ($t5)
        li $t8, IS_MRKD_MASK
        not $t9, $t8
        and $t6, $t6, $t9
        ...
        addi $t7, $t7, 1
        sw $t7, ($t6)
        j mark_cell__body_if3_end

        mark_cell__body_if3_false:
...
reveal_cell__body:
        ...
        #lb $t6, ($t5)           # value of grid[row][col]
        li $t6, 0 # TEMPORARY!
        andi $t7, $t6, IS_MRKD_MASK
        beq $t7, FALSE, reveal_cell__body_if1_false

        reveal_cell__body_if1_true:
        la $t6, debug_mode
        lw $t7, ($t6)
        beq $t7, FALSE, reveal_cell__body_if2_false

        reveal_cell__body_if2_true:
        j reveal_cell__epilogue

        reveal_cell__body_if2_false:
        la $a0, reveal_error
        li $v0, 4
        syscall
        j reveal_cell__epilogue

        reveal_cell__body_if1_false:

        #lb $t6, ($t5) 
        andi $t7, $t6, IS_RVLD_MASK
        beq $t7, FALSE, reveal_cell__body_if3_false

        reveal_cell__body_if3_true:
        la $t6, debug_mode
        lw $t7, ($t6)
        beq $t7, FALSE, reveal_cell__body_if4_false

        reveal_cell__body_if4_true:
        j reveal_cell__epilogue

        reveal_cell__body_if4_false:
        la $a0, already_revealed
        li $v0, 4
        syscall
        j reveal_cell__epilogue

        reveal_cell__body_if3_false:

        #lb $t6, ($t5) 
        andi $t7, $t6, IS_BOMB_MASK
        beq $t7, FALSE, reveal_cell__body_if5_false

        reveal_cell__body_if5_true:
        la $t6, game_state
        lw $t7, ($t6)
        li $t7, LOSE
        sw $t7, ($t6)

        reveal_cell__body_if5_false:

        #lb $t6, ($t5) 
        andi $t7, $t6, VALUE_MASK
        seq $t8, $t7, 0
        beq $t8, FALSE, reveal_cell__body_if6_else

        reveal_cell__body_if6_true:
        jal clear_surroundings

        reveal_cell__body_if6_else:
        #lb $t6, ($t5) 
        ori $t6, $t6, IS_RVLD_MASK
        #sb $t6, ($t5)
        ...
        la $t6, game_state
        lw $t7, ($t6)
        bne $t7, LOSE, reveal_cell__body_if7_false

        reveal_cell__body_if7_true:
        la $t8, cells_left
        lw $t9, ($t8)
        addi $t9, $t9, -1
        sw $t9, ($t8)

        reveal_cell__body_if7_false:

        reveal_cell__body_if6_end:

        bne $t9, 0, reveal_cell__body_if8_false

        reveal_cell__body_if8_true:
        la $t6, game_state
        lw $t7, ($t6)
        li $t7, WIN
        sw $t7, ($t6)

        reveal_cell__body_if8_false:
...
clear_surroundings__body:
        ...
        blt $s0, 0, clear_surroundings__body_if1_true
        bge $s0, N_ROWS, clear_surroundings__body_if1_true
        blt $s1, 0, clear_surroundings__body_if1_true
        blt $s1, N_COLS, clear_surroundings__body_if1_false

        clear_surroundings__body_if1_true:
        j clear_surroundings__epilogue

        clear_surroundings__body_if1_false:

        lb $s2, ($t5)
        andi $s3, $s2, IS_RVLD_MASK
        beq $s3, FALSE, clear_surroundings__body_if2_false

        clear_surroundings__body_if2_true:
        j clear_surroundings__epilogue

        clear_surroundings__body_if2_false:
        ...
        beq $t7, FALSE, clear_surroundings__body_if3_false

        clear_surroundings__body_if3_true:
        j clear_surroundings__epilogue

        clear_surroundings__body_if3_false:
...
update_highscore__body:
        ...
        bge $t1, $a0, update_highscore__body_if1_false

        update_highscore__body_if1_true:
        sw $a0, 0($t0)

        li $s0, 0   # int i = 0

        update_highscore__body_loop1_step:
        la $t2, user_name
        add $t2, $t2, $s0   # in this case, i acts as byte offset
        lb $t3, ($t2)

        #li $t8, '\0'
        beq $t3, $zero, update_highscore__body_loop1_end
        ...
        addi $s0, $s0, 1
        j update_highscore__body_loop1_step

        update_highscore__body_loop1_end:
        la $t4, 4($t0)
        add $t4, $t4, $s0   # in this case, i acts as byte offset
        #li $t5, '\0'
        sb $zero, ($t4)
        ...
        update_highscore__body_if1_false:
        li $v0, FALSE
...
print_scores__body:
        ...
        bge $s0, MAX_SCORES, print_scores__body_loop1_end
        print_scores__body_loop1_step:
        ...
        bne $t2, -1, print_scores__body_if1_false
        print_scores__body_if1_true:
        j print_scores__body_loop1_end

        print_scores__body_if1_false:
        ...
        addi $s0, $s0, 1
        j print_scores__body_loop1_step

        print_scores__body_loop1_end:
        la $a0, scores_line_msg
        li $v0, 4
        syscall
