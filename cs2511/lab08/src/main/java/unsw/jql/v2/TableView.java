...
public interface TableView extends Iterator<User>, Iterable<User> {
    ...
    /**
     * Filter a table view to another table view which contains a subset of the
     * records.
     *
     * Each item/record is passed through the filter, if the function returns true
     * then the record is kept otherwise it is excluded.
     *
     * An example would be `filter((fruit) -> fruit.age < 5)`
     */
    public TableView where(Predicate<User> pred);
     ...
     *
     * reducer: - First argument is the current accumulated value - Second argument
     * is the next item in the stream - Should return the next accumulated value.
     *
     * For example applying sum over ages (1, 2, 3, 4) is equal to ((((0 + 1) + 2) +
     * 3) + 4)
     */
    public <R> R reduce(BiFunction<R, User, R> reducer, R initial);

    /**
     * Apply reduce over multiple threads at once. This function has been written
     * for you but since your reduce isn't likely threadsafe you'll want to modify
     * your reduce to make it work with parallel reduce.
     */
    public <R> R parallelReduce(BiFunction<R, User, R> reducer, BinaryOperator<R> combiner, R initial,
