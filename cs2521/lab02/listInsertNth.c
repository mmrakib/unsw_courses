...
void listInsertNthRecur(struct node *node, int n, int value) {
	if (node == NULL) return;
	...
	if (n == 1) {
		...
		if (node->next != NULL)
			listInsertNthRecur(node->next, --n, value);
		else
			listInsertNthRecur(node, --n, value);
	}
}
...
void listInsertNth(struct list *list, int n, int value) {
	if (list->head == NULL) {
		...
		listInsertNthRecur(list->head, n, value);
	}
}
